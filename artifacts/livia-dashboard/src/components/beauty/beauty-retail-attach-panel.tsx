...
import { buildBeautyRetailPaySms } from "@workspace/policy";
import { Package } from "lucide-react";

type ProductOption = { id: string; name: string; priceMinor: number; currency: string };

export function BeautyRetailAttachPanel({
  businessId,
  businessName,
  guestFirstName,
  products,
  ...
  async function sendLink(productId: string) {
    ...
    try {
      ...
      const sms = buildBeautyRetailPaySms({
        businessName,
  ...
  return (
    <Card data-testid="beauty-retail-attach-panel">
      <CardHeader className="pb-2">
        <CardTitle className="text-base flex items-center gap-2">
          <Package className="h-4 w-4 text-primary" aria-hidden />
          Aftercare — mini store
        </CardTitle>
        <CardDescription>Generate a pay link to text after the session.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-2">
        ...
        <Button asChild size="sm" variant="ghost" className="px-0">
          <Link href="/beauty-store">Manage store</Link>
        </Button>
      </CardContent>
    </Card>
  );
